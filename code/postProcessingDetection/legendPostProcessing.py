...
def main():
    ...
    for imgName in testImageDir:
        ...
        # create polygon object for legend bbox and texts
        legendBbox = box(x1Legend, y1Legend, x2Legend, y2Legend)
        legendTextBboxes = [] # text bboxes in title
        for bound in textBboxes:
            textPolygon = box(bound[0][0][0],bound[0][0][1],bound[0][2][0],bound[0][2][1])
            if legendBbox.intersects(textPolygon) and bound[2]>pow(10,-10):
                legendTextBboxes.append(bound)
                print(bound[1])
