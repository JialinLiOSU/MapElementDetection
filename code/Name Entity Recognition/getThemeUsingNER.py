...
(
        ...
        {"entities": [(43, 84, theme),(93, 106, region),(108, 112, time),(113, 117, time)]},
),
(
...
def main():
    # extract title data
    titleList = []
    for textGood in TEST_DATA_Bad:
        
        title = textGood[0].replace("&", "and")
    ...
    for title in titleList:
        ...
        for ent in doc.ents:
            ...
            if ent.label_ == 'THEME':
                themeList.append(ent.text)
        if len(themeList) == 0:
            strtemp = strtemp + '\n'
            strList.append(strtemp)
            continue
        elif len(themeList) == 1:
            theme =  themeList[0] 
        else:
            theme = getBest(themeList)

        strtemp = strtemp + theme + '@'
        strtemp = strtemp + '\n'
        strList.append(strtemp)

    file = open(r'C:\Users\jiali\Desktop\MapElementDetection\code\Name Entity Recognition\themeResultsFinalBadTest.txt','a')
