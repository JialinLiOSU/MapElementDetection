...
def getCmapLegend(csIndex):# color scheme index
        if(csIndex == 4):
            return viridis
        elif(csIndex == 5):
            return plasma
...
def drawMapUnclassed(index, filename):
    ...
    # 11. if add title (No title in this case)
    title = getTitle()
    ...
    if (colorscheme >= 4):
        showLegend = 1
        loc_var = random.randint(1, 2)
        if (loc_var == 1):
            cmap = getCmapLegend(colorscheme)
            norm = mpl.colors.Normalize(vmin=5, vmax=10)
            cb= plt.colorbar(mpl.cm.ScalarMappable(norm=norm, cmap=cmap), orientation="horizontal",fraction=0.046, pad=0.04) 
        elif (loc_var == 2):
            cmap = getCmapLegend(colorscheme)
            norm = mpl.colors.Normalize(vmin=5, vmax=10)
            cb= plt.colorbar(mpl.cm.ScalarMappable(norm=norm, cmap=cmap), orientation="horizontal",fraction=0.046, pad=0.04) 
    ...
    plt.axis('off')
    # plt.show()
    bounds = cb.ax.figbox.bounds

    strLine = filename + ',' + str(bounds[0]) + ',' + str(bounds[1]) + ',' + str(bounds[2]) + ',' + str(bounds[3]) + ',1' + '\n'
...
def main():
    ...
    for i in range(0,5):
        ...
        filename = 'generated_unclassed_US_' + str(i) + '.png'
        # if(i >= 40 and i < 50):
        drawMapUnclassed(i, filename)
        # elif(i >= 15 and i < 30):
    ...
    filename='generated_unclassed_annotation_US'+'.txt'
