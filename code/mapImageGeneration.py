...
def getTitle():
    ...
    if (titleTypeID == 0):
        ...
        lenPhy = len(physical)
        return physical[random.randint(0,lenPhy-1)] + " in the US by state " + "in " + str(year)
    elif (titleTypeID == 3):
        lenSoc = len(social)
        return social[random.randint(0,lenSoc-1)] + " in the US by state " + "in " + str(year) 
    elif (titleTypeID == 4):
        lenHou = len(housing)
        return housing[random.randint(0,lenHou-1)] + " in the US by state " + "in " + str(year) 
    elif (titleTypeID == 5):
        lenRet = len(retailBusiness)
        return salesForRetail + retailBusiness[random.randint(0,lenRet-1)] + " in the US by state " + "in " + str(year)
    elif (titleTypeID == 6):
        lenCapExp = len(capitalExpenses)
        lenManType = len(manufacturingType)
        return capitalExpenses[random.randint(0,lenCapExp-1)] + connectWordManu + manufacturingType[random.randint(0,lenManType-1)] + " in the US by state " + "in " + str(year)
    elif (titleTypeID == 7):
        lenExpFirm = len(exportingFirms)
        return exportingFirms[random.randint(0,lenExpFirm-1)] + " in the US by state " + "in " + str(year) 
    elif (titleTypeID == 8):
        lenSchFin = len(schoolFinance)
        return schoolFinance[random.randint(0,lenSchFin-1)] + " in the US by state " + "in " + str(year) 
    elif (titleTypeID == 9):
        lenGovFin = len(governFinance)
        return governFinance[random.randint(0,lenGovFin-1)] + governFinancePost + " in the US by state " + "in " + str(year) 
    elif (titleTypeID == 10):
        lenHouHold = len(household)
        return household[random.randint(0,lenHouHold-1)] + " in the US by state " + "in " + str(year) 
    else:
        lenTimeUse = len(timeUse)
        lenTimeUseType = len(timeUseType)
        return timeUse[random.randint(0,lenTimeUse-1)] + timeUseType[random.randint(0,lenTimeUseType-1)] + " in the US by state " + "in " + str(year)
